...
async def get_deepseek_response(user_prompt: str, chat_history: list) -> AsyncGenerator[str, None]:
    ...
    system_prompt = ("""Ты — элитный AI-аналитик 'ND | Lookism'. Ты продолжаешь диалог с пользователем после 
                     ...
                     Отвечай на русском языке. ВАЖНО: Для форматирования текста всегда используй Telegram MarkdownV2.
                     • **Жирный** текст оформляй двойными звёздочками `**text**`.
                     • _Курсив_ — одинарным подчёркиванием `_text_`.
                     • Не используй HTML.
                     • Перед отправкой убеждайся, что ВСЕ специальные символы MarkdownV2 ( _ * [ ] ( ) ~ ` > # + - = | { } . ! ) экранированы обратным слешом `\`.
                     Если не уверен, отправь без разметки.
                     ОБЯЗАТЕЛЬНО следи, чтобы каждый HTML-тег был корректно ЗАКРЫТ и теги не пересекались (правильная вложенность).
                     Перед отправкой mentally проверь, что количество открывающих и закрывающих тегов одинаковое.
                     Если не уверен в корректности разметки, лучше отправь ответ без тегов.
